...
const exampleOperationsArray = [
  ...
  0,
  // profiling flags: BASIC + TIMELINE
  3,
...
describe("insertRootRemovalOpsForNavEvents", () => {
  // t00: Add root 2
  // t10: Add root 3
  // t20: Add root 4
  // t30: Remove root 2
  // t40: Add root 5
  const initialOps: OperationsInfo[] = [
    {
      point: "0",
      time: 0,
      originalOperations: [],
  ...
  const navEvents: ReplayNavigationEvent[] = [
    {
      point: "35",
      time: 35,
  ...
  test("Should insert root removal ops for navigation events", () => {
    const result = insertRootRemovalOpsForNavEvents(initialOps, navEvents);

    // There should have been 2 active roots at the time of
    // the nav event, so 2 root removal ops should have been inserted.
    const insertedRootRemovalOps = result.filter(info => info.point === "35");

    const expectedRootRemovalOps: OperationsInfo[] = [
      {
        point: "35",
        time: 35,
        originalOperations: [],
    ...
    expect(insertedRootRemovalOps).toEqual(expectedRootRemovalOps);
  });

  test("should not insert root removal ops if roots are used later", () => {
    const initialOpsWithExtraRootUsage = initialOps.concat({
      point: "50",
      time: 50,
      originalOperations: [],
      deconstructedOperations: {
    ...
    const result = insertRootRemovalOpsForNavEvents(
      initialOpsWithExtraRootUsage,
      navEvents
    );

    // Should not have added anything, because we saw that
    // one of the roots is used after the nav event
    expect(result).toEqual(initialOpsWithExtraRootUsage);
  });
});
